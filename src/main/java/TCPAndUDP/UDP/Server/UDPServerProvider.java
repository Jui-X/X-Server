...
public class UDPServerProvider {
    ...
    private static class Provider extends Thread {
        ...
        public void run() {
            ...
            try {
                datagramSocket = new DatagramSocket(Enum.UDP_PORT.getValue());
                ...
                while (done) {
                    ...
                    // 校验接收到的数据是否有误
                    boolean isValid = (dataSize > Constant.HEADER.length + Enum.SHORT_LENGTH.getValue() + Enum.INT_LENGTH.getValue())
                            && ByteUtils.startsWith(clientData, Constant.HEADER);
                    ...
                    int index = Constant.HEADER.length;
                    ...
                    if (cmd == Enum.CMD_RECEIVE.getValue() && resPort > 0) {
                        ByteBuffer byteBuffer = ByteBuffer.wrap(buffer);
                        byteBuffer.put(Constant.HEADER);
                        byteBuffer.putShort((short) Enum.CMD_SEND.getValue());
