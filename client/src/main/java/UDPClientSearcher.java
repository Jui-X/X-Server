...
public class UDPClientSearcher {
    private static final int RESPONSE_PORT = UDP_RESPONSE_PORT.getValue();
    ...
    private static class Listener extends Thread {
        ...
        private final int minLen = HEADER.length + SHORT_LENGTH.getValue() + INT_LENGTH.getValue();
        ...
        public void run() {
            ...
            try {
                ...
                while (done) {
                    ...
                    ByteBuffer byteBuffer = ByteBuffer.wrap(buffer, HEADER.length, dataSize);
    ...
    private static void broadcast() throws IOException {
        ...
        // cmd命令
        byteBuffer.putShort((short) CMD_RECEIVE.getValue());
        // 回送端口
        ...
        // 设置服务器接收UDP请求的端口
        requestPacket.setPort(UDP_PORT.getValue());
